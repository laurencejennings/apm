...
from apm_streamlit_app.config.config_loader import read_config_from_path, CONFIG_PATH
from apm_streamlit_app.utils.caching_functions import cache_kpe_pickle, cache_comments_of_subreddit
...
BASE_S3_URL = "https://sagemaker-eu-west-1-100701698608.s3.eu-west-1.amazonaws.com/reddit_graph_data/kratika-test-3"

config = read_config_from_path(CONFIG_PATH)
GRAPH_DATE = get_date_n_days_before_today(config.get('dates')['days_before_today'])

comments_kpes_per_subreddit = cache_kpe_pickle(GRAPH_DATE)
...
with main_Section:
    ...
    st.markdown("**Below are the colours of the verticals as provided by PET.**")
    ...
    with st.expander('Explore subreddit topics', expanded=False):
        ...
        with c1:
            subreddit_to_explore = st.selectbox(
                ...
                [None] + subreddits_with_topics)

            if subreddit_to_explore is not None:
                subreddit_kps = list(comments_kpes_per_subreddit[subreddit_to_explore].keys())[2:]
                if len(subreddit_kps):
                    ...
                    if kp_to_explore:
                        ...
                        comments = cache_comments_of_subreddit(
                            comments_kpes_per_subreddit[subreddit_to_explore][kp_to_explore])
                        ...
                        if len(comments):
                            with c2:
                                comments_of_page = paginator(
                                    ...
                                    comments)
                                comments_data = list(
                                    zip(comments_of_page.body, comments_of_page.link_id, comments_of_page.id))
                                for comment in comments_data:
                                    ...
                                    highlighted_comment = re.sub(
                                        '(?i)(' + '|'.join(map(re.escape, [kp_to_explore])) + ')',
                                        '<span style="color:Red;font-size:20px;">' + r'\1' + '</span>', comment[0])
    ...
    with c2:
        ...
        with st.form("main_form"):
            ...
            graph_file_name = create_file_name_from_params('association_graph', min_items, min_conf, min_interest, size)

            (url, plotted_graph_date, errors) = generate_url(GRAPH_DATE, graph_file_name)
...
with plot_Section:
    ...
    plot_Section.markdown(
        "**Community Detection graph** is where the algorithm clusters communities together automatically based on edge strength. The different clusters are shown with different colors.")
    plot_Section.markdown(
